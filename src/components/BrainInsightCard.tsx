...
export default function BrainInsightCard({ insight, defaultExpanded = false }: Props) {
  ...
  const sourceEvents = insight.metadata?.source_events ?? [];
  const isBrain2 = insight.metadata?.formula?.startsWith('brain2_');
    ...
    <article className="card-elevated">
      ...
      >
        ...
        <div className="flex items-center justify-between mt-3">
          <span className="text-xs text-chef-subtle">
            Confidence: {confidencePct}%{isBrain2 ? ' · Brain 2.0' : ''}
          </span>
      ...
      {expanded && (evidence.length > 0 || sourceEvents.length > 0) && (
        <div className="mt-3 pt-3 border-t border-steel">
          ...
          <ul className="space-y-1">
            {evidence.map((line, i) => (
              <li key={`e-${i}`} className="text-sm text-chef-subtle flex items-start gap-2">
                <span className="text-chef mt-0.5">•</span>
                {line}
              </li>
            ))}
            {sourceEvents.slice(0, 4).map((evt, i) => (
              <li key={`s-${i}`} className="text-xs text-chef-muted flex items-start gap-2 font-mono">
                <span className="text-chef mt-0.5">↳</span>
                {evt.startsWith('ledger:') ? `Decision ledger ${evt.replace('ledger:', '')}` : evt}
              </li>
            ))}
          </ul>
        </div>
