...
export default function Assistant() {
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-140px)]">
      <h2 className="font-display text-xl text-chef-800 mb-3">{profile?.assistant_name || 'Sous Chef'}</h2>
          ...
          >
            <div
              className={`max-w-[85%] rounded-2xl px-4 py-3 ${
                msg.role === 'user'
                  ? 'bg-chef-500 text-white rounded-br-md'
                  : 'bg-white border border-sage-100 rounded-bl-md'
              }`}
            >
              <p className="text-sm">{msg.content}</p>
              {msg.role === 'assistant' && (
                <button onClick={() => speak(msg.content)} className="mt-1 text-sage-400 hover:text-chef-500">
                  <Volume2 size={14} />
                </button>
              )}
            </div>
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-white border border-sage-100 rounded-2xl px-4 py-3">
              <p className="text-sm text-sage-400 animate-pulse">Thinking...</p>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {quickPrompts.map((p) => (
          <button key={p} onClick={() => send(p)} className="text-xs bg-sage-100 text-sage-700 px-3 py-1.5 rounded-full">
            {p}
          </button>
        ))}
      </div>

      <div className="flex gap-2 items-center bg-white border border-sage-200 rounded-2xl p-2">
        ...
        />
        <button onClick={() => send(input)} disabled={loading || !input.trim()} className="btn-icon bg-chef-500 text-white">
          <Send size={18} />
        </button>
      </div>
    </div>
